...
export default function SearchResults() {
  ...
  Axios.get("http://localhost:3010/users/isUserAuthenticated",{headers:{"x-access-token":Item["token"]},}).then((res)=>{
    ...
    if(!vehicleClass && !makeName && !city && !MyState )
    {
        ...
        Axios.get("http://localhost:3010/listings/get_allListings").then((res)=>{
     ...
     else if(MyState)
     {
      console.log("image.png"+MyState);
          ...
          {
           Axios.post("http://localhost:3010/listings/search_bar",{keyword:MyState.showresult,location:keywordLocation["SearchBarLocation"]}).then((res)=>{
              console.log(res.data);
              setfetchListings(res.data);
              });
          }
            ...
            Axios.get("http://localhost:3010/categories/getprices").then((res2)=>{
            //  console.log(res2.data);
              setvehicleprice(res2.data);
            });
            Axios.get("http://localhost:3010/categories/years").then((res3)=>{
  ...
  useEffect(()=>{
    ...
    { 
      if(type=="private")
      {
        Axios.post("http://localhost:3010/listings/FilterByListingSellertype",{ListingSellertype:"private"}).then((res22)=>{
      ...
      {
        Axios.post("http://localhost:3010/listings/FilterByListingSellertype",{ListingSellertype:"dealer"}).then((res22)=>{
          console.log(res22.data);
        setfetchListings(res22.data);
         });
      }
      ...
      {
        Axios.post("http://localhost:3010/listings/FilterByListingSellertype",{ListingSellertype:"sold"}).then((res22)=>{
          console.log(res22.data);
        setfetchListings(res22.data);
         });
      }
    ...
    if(city)
    {
      ...
      Axios.post("http://localhost:3010/listings/FilterByCityListings",{cityname:city}).then((res11)=>{
    ...
    console.log("hanji kidan");
      Axios.post("http://localhost:3010/listings/FilterByClassListings",{vehicleClass:vehicleClass}).then((res22)=>{
        console.log(res22.data);
      setfetchListings(res22.data);
       });
    }
    ...
    console.log("hanji makeName");
      Axios.post("http://localhost:3010/listings/FilterByMakeListings",{makename:makeName}).then((res33)=>{
    ...
    console.log("hanji MyState");
      Axios.post("http://localhost:3010/listings/Find_A_Car",MyState).then((res43)=>{
...
const PushArr=(e)=>
{
  if(e.length > 0)
  {
    Axios.post("http://localhost:3010/categories/getchoosedmakes",{makesarr:e}).then((res4)=>{
  ...
  else if(AllMakes.length == fetchMakes.length)
  {
     ...
     Axios.post("http://localhost:3010/categories/getchoosedmakes",{makesarr:AllMakes}).then((res4)=>{
      console.log(res4.data);
      setfetchModels(res4.data); 
    });
  }
...
const onAllMakes=()=>{
  alert(fetchMakes1.length);
  fetchMakes1.length=0;
  settogg(!togg);
  handleAllMakes();
}
const handleSellerId= (e)=>
{
  ...
  Axios.post("http://localhost:3010/users/getDealerDetails",{sellerid:e}).then((res4)=>{
      console.log(res4.data);
...
const handleAllMakes=()=>
{
  ...
  if(!togg){
    fetchMakes1.length=0;
    alert("INSIDE Toggle Check"+togg + "fetch Makes length"+fetchMakes1.length);
    setlistVal("");
    
     fetchMakes.map((Val)=>{
     setAllMakes(AllMakes=>[...AllMakes,Val.makes_name]);
     });
    }
    else if(togg)
      {
...
const searchMake=(e)=>
{
   ...
   const Filtered_Makes=Happy1.filter((val)=>{
    return val.makes_name.toLowerCase().includes(debouncedMake.toLowerCase());
   });
...
const searchModel=(e)=>
{
  setsearchedModel(e);
   ...
   const Filtered_Model=Happy2.filter((val)=>{
    return val.model_name.toLowerCase().includes(debouncedModelQuery.toLowerCase());
   });
...
const Apply=()=>
{
    ...
    const a=minprice.length;
    const b=maxprice.length;
    const aa=minprice.slice(1,a);
    const bb=maxprice.slice(1,b);
    ...
    if(query1)
    {
      ...
      if(a>0)
      {
        Axios.post("http://localhost:3010/listings/getFilterListings",{buyfromHome:buyfromHome,cityname:query1,bodyStyle:bodyStyle,allmodels:allModels,allmakes:AllMakes,makename:fetchMakes1,modelname:fetchModels1,minyears:minyears,maxyears:maxyears,minprice:aa,maxprice:bb,minmileage:minmileage,maxmileage:maxmileage,transmission:sel,drivetrain:drivetrain,used_new:sel1}).then((res)=>{
          console.log(res.data);
          setfetchListings(res.data);
         });
      }
      ...
      {
        Axios.post("http://localhost:3010/listings/getFilterListings",{buyfromHome:buyfromHome,province:query1,bodyStyle:bodyStyle,allmodels:allModels,allmakes:AllMakes,makename:fetchMakes1,modelname:fetchModels1,minyears:minyears,maxyears:maxyears,minprice:aa,maxprice:bb,minmileage:minmileage,maxmileage:maxmileage,transmission:sel,drivetrain:drivetrain,used_new:sel1}).then((res)=>{
          console.log(res.data);
          setfetchListings(res.data);
         });
      }
 ...
 const [sortFilter,setsortFilter]=useState('');
   ...
   const SearchKeyword=(e)=>
   {
      if(e.key==="Enter")
      {
        Axios.post("http://localhost:3010/listings/search_bar",{keyword:e.target.value,location:query1}).then((res)=>{
          console.log(res.data);
          setfetchListings(res.data);
          });
      }
                      ...
                      {show4 ?<div>
                          <div className='srch-rs-row srch-rs-lst fd-c' style={{marginTop:"10px"}}>
                            ...
                            { fetchMakes.map((val)=>{
                                        ...
                                        return <li className={fetchMakes1.includes(val.makes_name) || AllMakes.length > 0 ?'liit lab-lnk':"liit"}  >
                                          <input type="checkbox" name="make12" className='example'   id={val.makes_name}  
                                            
                                            onChange={(e)=>{setlistVal(e.target.id);setlistStyle(e.target.checked);settoggle1(!toggle1)}} />
                                          {AllMakes.length > 0 ?<BsCheck className='ico-6 ' ></BsCheck>:fetchMakes1.includes(val.makes_name) ?<BsCheck className='ico-7 ' ></BsCheck>:""}
                                          <label  className={AllMakes.length > 0 || fetchMakes1.includes(val.makes_name) ?'make_label1':'make_label'}  htmlFor={val.makes_name} >{val.makes_name}
                                          </label>
                                            </li>
                                      })
                                    }
                         ...
                         <li className={allModels.length > 0 ?'liit lab-lnk':'liit'} ><input type="checkbox" name="make123" className='example'  id="All Models"  
                                ...
                                { fetchModels.map((val)=>{
                                        return <li className={fetchModels1.includes(val.model_name) || allModels.length  ?'liit lab-lnk':"liit"}>
                                          <input type="checkbox" name="make123" className='example'    id={val.model_name}  
                                            onChange={(e)=>{setlistVal1(e.target.id);setlistStyle1(e.target.checked);settoggModels(!toggModels)}} />
                                         {allModels.length > 0 ?<BsCheck className='ico-6 ' ></BsCheck>:fetchModels1.includes(val.model_name) ? <BsCheck className='ico-7'   ></BsCheck>:""}
                                          <label  className={allModels.length > 0 || fetchModels1.includes(val.model_name) ?'make_label1':'make_label'}  htmlFor={val.model_name} >{val.model_name}
                                          </label>
                                            </li>
                                      })
                        ...
                        <div className='srch-rs-row fd-r align-center srch-rs-lst'>
                              ...
                              <option value="" selected="selected"  >MIN</option>
                              {
                                vehicleprice.map((val)=>{
                                  return <option value={val.prices} >{val.prices}</option>
                                })
                              }
                              </select>
                              </div>
