...
export default function ShowDealer() {
    ...
    const [query1, setQuery1] = useState(state?state.dealer_Location_state:"");
    ...
    const [fshowDealer, setfshowDealer] = useState(false);
  ...
  const [UserDeleteId , setUserDeleteId]= useState('');
   ...
   // console.log("001"+state.dealer_Location_state);
    ...
    useEffect(()=>{
      ...
      {
        setLocName();
        console.log("state.dealer_Location_state"+JSON.stringify(state));
        keywordLocation = JSON.parse(localStorage.getItem('User_Choice_Location'));
        if(state.showDeal)
        {
          Axios.post("https://carpages-canada-mongodb.onrender.com/users/search_bar",{keyword:state.showDeal,location:keywordLocation["SearchBarLocation"]}).then((res)=>{
        ...
        {
          Axios.post("https://carpages-canada-mongodb.onrender.com/users/getDealerByAlphabet",{alphabet:state.dealer_alphabet}).then((res)=>{
            console.log(res.data);
            setshowDealerList(res.data);
            });
        }
        if(state.dealer_Location_state)
        {
          console.log("state.dealer_Location_state Working");
         setArea(state.dealer_Location_state);
        }
      ...
      else if(!state)
      {
        ...
        Axios.get("https://carpages-canada-mongodb.onrender.com/users/allUsers").then((res)=>{
            console.log(res.data);
            setshowDealerList(res.data);
            });
      }
     ...
     if(LocName==="")
      ...
      if(type)
      {
        Axios.post("https://carpages-canada-mongodb.onrender.com/users/getDealerByType",{userType:type}).then((res)=>{
            console.log(res.data);
            setshowDealerList(res.data);
            });
    ...
    const changeUserStatus=(ID,status)=>
   {
     ...
     if(status==="unblock")
     {
       Axios.post("https://carpages-canada-mongodb.onrender.com/users/updatetUserStatus",{UserId:ID,status:"block"}).then ((response1) =>{
     ...
     {
       Axios.post("https://carpages-canada-mongodb.onrender.com/users/updatetUserStatus",{UserId:ID,status:"unblock"}).then ((response1) =>{
         console.log(response1.data);
    ...
    const DealersData=()=>
    {
        ...
        console.log(a+'....'+b+"..."+c+"..."+d+"..."+f+"....");
            if(LocName ==="")
            {
                Axios.post("https://carpages-canada-mongodb.onrender.com/users/allDealers",{dealer:"dealer"}).then((res1)=>{
            ...
            else if(LocName.length>0)
            {
                if(a>0)
                {
                    Axios.post("https://carpages-canada-mongodb.onrender.com/users/dealerbycity",{city_name:LocName}).then((res1)=>{
                    console.log(res1.data);
                    setshowDealerList(res1.data);
                    });
                }
                else if(a<0)
                {
                    ...
                    Axios.post("https://carpages-canada-mongodb.onrender.com/users/dealerbyprovince",{province:LocName}).then((res1)=>{
                    console.log(res1.data);
                    setshowDealerList(res1.data);
                    });
                }
     ...
     function useDebouncedValue1(value, wait) {
      ...
      const [query2, setQuery2] = useState(state?state.dealer_Name_state:"");
   ...
   {      setshowlocations(true);
    ...
    const ShowDealers =()=>
    {
      ...
      setshowDealer(true);
      //setfshowDealer(true);
      console.log(showDealer+"LIST.."+JSON.stringify(dealerslist));
    }
    const pasteDealerName =(e)=>
    {
      console.log("Paste Name"+e+"Query2"+query2);
      setQuery2(e);
      setshowDealer(false);
      setfshowDealer(false);
    }
   ...
   if(a>0)
   {
    Axios.post("https://carpages-canada-mongodb.onrender.com/users/dealerbycity",{cityname:e}).then((res1)=>{
      console.log(res1.data);
      setshowDealerList(res1.data);
    });
   }
   else if(a<0)
   {
    ...
    Axios.post("https://carpages-canada-mongodb.onrender.com/users/dealerbyprovince",{province:e}).then((res1)=>{
      console.log(res1.data);
      setshowDealerList(res1.data);
    });
   }
  ...
  const refineSearch=()=>
  {
     ...
     {  
      ...
      {
        //alert("if 1"+query1);
       Axios.post("https://carpages-canada-mongodb.onrender.com/users/getDealerByCity",{city_name:query1,dealer_name:query2}).then((res1)=>{
         console.log(res1.data);
         setshowDealerList(res1.data);
       });
      }
      else if(a<0  && query2.length>0)
      {
       ...
       Axios.post("https://carpages-canada-mongodb.onrender.com/users/getDealerByProvince",{province:query1,dealer_name:query2}).then((res1)=>{
         console.log(res1.data);
         setshowDealerList(res1.data);
       });
      }}
      ...
      {
        //alert("else if 2"+query1);
        Axios.post("https://carpages-canada-mongodb.onrender.com/users/getDealerByOnlyName",{dealer_name:query2}).then((res1)=>{
          console.log(res1.data);
          setshowDealerList(res1.data);
        });
      }  
    ...
    <div className='shd-maincont'>
            ...
            <div className='shd-subcont'>
                <div className='shd-sidemenu  '>
                    ...
                    <hr className='shd-hr' />
                    <div className='shd-sidemenu-opt shd-dsp shd-fd shd-mg-bott' >
                        <label htmlFor="" className='shd-sidemenu-lbl shd-mg-bott'>DEALER</label>
                        <ClickAwayListener   onClickAway={handleClickAway1}> 
                        <div style={{position:"relative"}}>
                        <input style={{width:"100%"}} type="text" placeholder='Dealer Name' value={query2} className='shd-inp shd-mg-bott' onClick={()=>{ShowDealers()}}  onChange={(e)=>{getDealers(e.target.value);setshowDealer(true)}}/>
                        {fdealerslist.length >0 ?<div className='fd-filter-suggestion'>
                             <div className="fd-filter-suggestions-deal" >
                             { fshowDealer ? fdealerslist.slice(0,5).map((val)=>{
                             ...
                             <div className="fd-filter-suggestions-deal" > 
                             {showDealer? showDealerList.map((val)=>{
                              return  <div style={{fontWeight:"600"}} className='fd-filter-suggestions-item' onClick={()=>{pasteDealerName(val.user_name)}}>{val.user_name}</div>})
                           :""                            
                            }
                              </div>
                               </div>}  
